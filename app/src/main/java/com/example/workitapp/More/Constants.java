package com.example.workitapp.More;

import com.example.workitapp.R;

public class Constants {


    public static final String SP_FILE = "SP_FILE_1";
    public static final String REMEMBER = "REMEMEBR"; // key to remember user
    public static final String UID = "REMEMEBRED_UID"; // key to the remembered user id
    public static final String DIVISION_PATH = "Divisions";
    public static final String WORKER_PATH = "Workers";
    public static final String REQUESTS_PATH = "Requests";
    public static final String ASSIGNMENTS = "assignments";
    public static final String IS_ACCEPTED = "isAccepted";

    public static final int MANAGER_ID = 0;
    public static final int WORKER_ID = 1;
    public static final int HR_ID = 2;

    public static final int MY_CAMERA_REQUEST_CODE = 100;
    public static final int READ_EXTERNAL_STORAGE_CODE = 101;

    public static final String PROFILE_FOLDER = "profile_pics/";
    public static final String DEFAULT = "default";
    public static final int PROFILE_DEFAULT = R.drawable.profile_icon;
    public static final String IMG_URL = "imgUrl";
    public static final String UNAUTH_TEXT = "Sorry, ";
    public static final String AUTH_TEXT = "Hello, ";
    public static final int LOGO_WHITE = R.drawable.logo_white;
    public static final int UNAUTH_PIC = R.drawable.unauth_pic;
    public static final int AUTH_PIC = R.drawable.auth;
    public static final String AUTH_ACCEPTED = "You Have \nBeen Accepted!";
}
